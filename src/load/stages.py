...
from src.load.postgres import connect, record_run, upsert_curated
...
def record_failure(run_id, started_at, counts, message):
    try:
        ...
        logger.warning('could not record the failure of %s in audit.pipeline_runs: %s', run_id, audit_error)
