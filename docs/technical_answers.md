...
The hash must exclude anything that changes without the source data changing. `pipeline_run_id` and `processed_at_utc` differ on every run, so including them would give every row a new hash and force every row to be rewritten. `source_updated_at` is included, because it identifies which version of the source record the row came from: a new source version, even one that changes only `updated_at` as for `O0010000`, changes the hash and updates the stored row, so the stored `source_updated_at` never lags behind the source. Two curated runs with different run IDs produced identical hashes for every row (`docs/evidence/goal2_curated.txt`, recorded before `source_updated_at` joined the hash), which is the property that makes a second load over unchanged source data a no-op.
